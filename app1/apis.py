...
def manage_class(request):
    ...
    if request.method == 'POST':
        try:
            ...
            c = Class(name=name)
            c.save()
            return JsonResponse(response_dict(code=0, msg='添加成功'))
        except (JSONDecodeError, KeyError, TypeError):
            return JsonResponse(response_dict(code=500, msg='参数错误', success=False))
        except IntegrityError:
            return JsonResponse(response_dict(code=300, msg='班级已存在', success=False))
    ...
    # 删除班级
        ...
        try:
            ...
            if result[0] == 0:
                return JsonResponse(response_dict(code=100, msg='班级不存在', success=False))
            elif result[0] == 1:
                return JsonResponse(response_dict(code=0, msg='删除成功'))
        except ProtectedError as e:
            if 'Student' in str(e):
                return JsonResponse(response_dict(code=400, msg='该班级下存在学生，不可删除', success=False))
        except ValueError:
            return JsonResponse(response_dict(code=500, msg='参数错误', success=False))
    ...
    # 更新班级
        ...
        try:
            ...
            if result == 1:
                return JsonResponse(response_dict(code=0, msg='修改成功'))
            elif result == 0:
                return JsonResponse(response_dict(code=100, msg='班级不存在', success=False))
        except (JSONDecodeError, KeyError, TypeError):
            return JsonResponse(response_dict(code=500, msg='参数错误', success=False))
    ...
    # 查询班级
        ...
        try:
            ...
            return JsonResponse(response_dict(code=0, msg='查询成功', data=data))
